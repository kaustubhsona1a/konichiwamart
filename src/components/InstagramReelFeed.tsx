...
export const InstagramReelFeed: React.FC<InstagramReelFeedProps> = ({
  ...
  return (
    <section className="relative py-8 sm:py-14 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-4 sm:mb-6 gap-3 sm:gap-4 text-left">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full bg-pink-100 border border-pink-200 text-pink-800 text-[10px] sm:text-xs font-semibold tracking-wide mb-1.5 sm:mb-2">
          ...
          <p className="text-xs sm:text-sm text-slate-600 font-normal mt-0.5 sm:mt-1 max-w-xl leading-relaxed">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-white hover:bg-slate-50 text-slate-800 text-xs font-semibold border border-slate-200 shadow-xs transition-all cursor-pointer"
          >
            ...
            <span className="text-[10px] sm:text-[11px] text-slate-500 font-normal">• 124K</span>
          </a>
        </div>
      </div>
      ...
      >
        {REELS.map((reel) => {
          const featuredProduct = PRODUCTS.find(p => p.id === reel.productId) || PRODUCTS[0];
        ...
        <div className="fixed inset-0 z-50 bg-pink-950/45 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-[380px] h-[88vh] max-h-[720px] rounded-3xl overflow-hidden bg-slate-950 shadow-2xl border border-pink-300/30 flex flex-col justify-between">
            ...
            <div className="relative z-20 p-4 pt-0 space-y-3 text-white text-left">
              ...
              {(() => {
                const prod = PRODUCTS.find(p => p.id === activeReelModal.productId) || PRODUCTS[0];
                return (
                  <div className="bg-white/95 text-slate-900 border border-pink-200 rounded-2xl p-2.5 flex items-center justify-between gap-3 shadow-2xl backdrop-blur-md">
                    <div 
                      onClick={() => {
                        closeReelModal();
                    ...
                    >
                      <img
                        src={prod.image}
                        alt={prod.title}
                        className="w-11 h-11 rounded-xl object-contain bg-pink-50 border border-pink-100 p-1"
                        referrerPolicy="no-referrer"
                      />
                      <div className="truncate">
                        <div className="text-[10px] text-pink-600 uppercase tracking-wider font-bold">Featured Product</div>
                        <div className="text-xs font-bold text-slate-900 truncate">{prod.title}</div>
                        <div className="text-xs font-black text-pink-600">{formatINR(prod.price)}</div>
                      </div>
                    </div>

  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white border-l border-slate-200 shadow-2xl flex flex-col justify-between text-left">
          
          {/* Drawer Header */}
          <div className="p-4 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-pink-100 text-pink-700 flex items-center justify-center border border-pink-200 shadow-xs">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-900 transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="px-4 py-2.5 bg-pink-50/70 border-b border-pink-100 space-y-1.5">
            <div className="flex items-center justify-between text-xs text-slate-700">
              <span className="flex items-center gap-1.5 font-medium">
                <Truck className="w-3.5 h-3.5 text-pink-500" />
                {amountNeededForFree === 0 ? (
                  <strong className="text-emerald-700">Unlocked: Free Delivery!</strong>
                ) : (
                  <span>Add <strong>{formatINR(amountNeededForFree)}</strong> more for Free Delivery</span>
                )}
              </span>
              <span className="text-[11px] font-mono text-slate-500">{Math.round(progressToFree)}%</span>
            </div>
              ...
              items.map((item) => {
                ...
                return (
                  <div
                    ...
                    className="p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-3 shadow-xs"
                  >
                    <img
                      src={item.product.image}
                      alt={item.product.title}
                      className="w-14 h-14 object-contain rounded-lg bg-white border border-slate-200 p-1 flex-shrink-0"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        {/* Quantity controller */}
                        <div className="flex items-center rounded-lg bg-white border border-slate-200 px-1 py-0.5 shadow-xs">
                          <button
                            onClick={() => onUpdateQuantity(item.product.id, -1, shadeId)}
                            className="w-5 h-5 rounded hover:bg-slate-100 flex items-center justify-center text-slate-600 hover:text-slate-900 cursor-pointer"
                          >
                        ...
                        {/* Price */}
                        <div className="font-bold text-sm text-pink-700">
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1.5">
                ...
                <form onSubmit={handlePincodeCheck} className="flex items-center gap-1.5">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-mono text-slate-900 outline-none focus:border-pink-500"
                  />
                  <button
                    type="submit"
                    className="px-3 py-1.5 rounded-lg bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium cursor-pointer shadow-xs"
                  >
                    Check
                  </button>
                </form>
            ...
            {items.length > 0 && (
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                <div className="flex items-center justify-between text-xs font-semibold text-slate-800">
                  ...
                  {appliedPromo && (
                    <span className="text-[10px] bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-0.5 rounded-md font-semibold">
                ...
                <form onSubmit={handleApplyPromo} className="flex items-center gap-1.5">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs text-slate-900 uppercase font-medium tracking-wide outline-none focus:border-pink-500"
                  />
                  <button
                    type="submit"
                ...
                <div className="flex items-center gap-1.5 pt-0.5">
                  ...
                  <button 
                    ...
                    className="text-[10px] bg-white hover:bg-pink-50/50 text-pink-700 px-2 py-0.5 rounded border border-slate-200 cursor-pointer shadow-xs"
                  >
                    GLOW15 (15% off)
                  </button>
                  <button 
          ...
          {items.length > 0 && (
            <div className="p-4 border-t border-slate-200 bg-slate-50 space-y-2.5">
              ...
              <div className="space-y-1 text-xs text-slate-600">
                ...
                <div className="pt-2 border-t border-slate-200 flex justify-between items-baseline text-sm font-semibold text-slate-900">
